...
import { checkConnection, CordenadasActualesNumerico, ObtenerDireccionActual } from '../../utilities/utilities';
...
import { OK,DESCONOCIDO,WIFI_OFF, INFO, ERROR, CAMERA} from '../../Styles/Iconos';
...
const itemWidth = slideWidth + itemHorizontalMargin * 2;
  const _renderItem = ({ item, index }) => {
    return (
      ...
      >
          <TouchableOpacity onLongPress={ ()=>{eliminarEvidencia(item.uri)} } >
            <Card>
              <Image
                source={{ uri: item.uri }}
  ...
  const obtenerDireccionActuales = async () => {
    ...
    //NOTE: Obtenemos los 
    let DireccionActual = JSON.parse(await ObtenerDireccionActual(coordenadasActuales));
    let formatoDireccion = `
      Estado: ${DireccionActual.region}
      Ciudad: ${DireccionActual.city}
      Colonia: ${DireccionActual.district}
      ...
      setDireccion(formatoDireccion);
      setDireccionEnviar(`${DireccionActual.region}  ${DireccionActual.city} ${DireccionActual.district} ${DireccionActual.street} ${DireccionActual.postalCode}`);
      console.log(`${DireccionActual.region} ${DireccionActual.city} ${DireccionActual.district} ${DireccionActual.street} ${DireccionActual.postalCode}`);
    setLoading(false);
  }
  ...
  const eliminarEvidencia = (uri:String) => {
    //NOTE: lanzamos el modal para eliminar
      Alert.alert("Mensaje"," ¿ Eliminar Imagen ?",[
        {
          text: "Aceptar",
          onPress:()=>{ setArrayImageEncode(arrayImageEncode.filter(item => item.uri !== uri )); },
          style:"cancel"
        },
        {
          text:"Cancelar",
    ...
    <View style = { [Styles.container]}>
      {
        ...
        <ScrollView contentContainerStyle = {{flexGrow:1}} >
          <View style = {Styles.cardContainer} >
              <View style = {[ errorUi.includes("T,") ? Styles.errorDatos : {} ,{flex:.2}]} >
                {/* NOTE:: Area Administrativa */ }
                <Picker
                  selectedValue={seleccionSolicitud} 
                  onValueChange = {(itemValue, itemIndex)=>{setSeleccionSolicitud(String(itemValue))}} > 
                  <Picker.Item label=" Temario de solicitud " value={ -1 } ></Picker.Item>
                  {
                    catalogoSolicitud.map((item,index)=>{
                      return <Picker.Item key={item.id} label = { item.descripci_on } value={ item.id } ></Picker.Item>
                    })
                  }
                </Picker>
              </View>
              ...
              <View style = {{ flex:6 }} >
                ...
                <Button 
                  ...
                  buttonStyle = {[Styles.btnButtonSuccessSinPading, errorUi.includes("E,") ? Styles.errorDatos : {}]} />
                  <View>
                    <TextInput 
                      ...
                      style = {[Styles.bachesTextInput, errorUi.includes("R,") ? Styles.errorDatos : {}]} ></TextInput>
                  </View>
                  <View>
                    <TextInput 
                      ...
                      style = {[ Styles.bachesTextInput, errorUi.includes("D,") ? Styles.errorDatos : {}]}></TextInput>
                  </View>
                  <Button 
                  icon={{
                    name: 'save',
